...
- `[...]` vs. `[[...]]`
= `==` vs. `=` 
- Comparing real numbers exactly using `==` is dangerous (more
in a later Unit). Suppose you generate `x = 0.333333` in some
fashion with some code and then check: `x == 1/3`. This will produce FALSE.
...
having the function, via R's scoping rules, get that variable as a global
...
fine when you develop the code (if the variable exists in the enclosing
environment), but then may not work when you restart R if the variable
no longer exists or is different.
...
of the code (w/ backups of course) so you only need to change it once
...
you can easily induce a bug if you forget the order of ops.
- Documentation - add lots of comments (but don't belabor the obvious).
Remember that in a few months, you may not follow your own code any
better than a stranger. Some key things to document: 
...
c
```
- Use active names for functions (e.g., \emph{calc_loglik}, \emph{calcLogLik})
...
- [Gentzkow and Shapiro tutorial for social scientists](faculty.chicagobooth.edu/jesse.shapiro/research/CodeAndData.pdf)
...
* If feasible, include your code for doing analyses and making figures in the relevant document reporting the work by using R Markdown or *Latex* with *knitr* or *Sweave*. 
