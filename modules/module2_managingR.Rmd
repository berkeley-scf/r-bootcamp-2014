...
If you're not using the VM, now (or at the break) would be a good point to install the various packages we need for the bootcamp, which can be done easily with the following command:

```{r eval=FALSE}
install.packages(c('RSQLite', 'plyr', 'reshape2', 'ggplot2', 'chron',
                              'gridExtra', 'codetools', 'rbenchmark',
                              'lme4', 'doParallel', 'foreach', 'xtable',
                              'fields'),  repos = "http://cran.cnr.berkeley.edu")
```
...
# Linux/Mac specific
setwd('~/Desktop/r-bootcamp-2014') # change the working directory
setwd('/Users/paciorek/Desktop/tmp') # absolute path
getwd()
setwd('../r-bootcamp-2014') # relative path

# platform-agnostic
setwd(file.path('~', 'Desktop', 'r-bootcamp-2014')) # change the working directory
setwd(file.path('/', 'Users', 'paciorek', 'Desktop', 'tmp')) # absolute path
getwd()
setwd(file.path('..', 'r-bootcamp-2014')) # relative path
```
...
Git is a popular tool for version control. Git is based around the notion of a repository, which is basically a version-controlled project directory. Many people use it with the Github online hosting service for repositories.
...
We'll go through a short example of making changes to the r-bootcamp-2014 repository. In this case you don't have permission to make changes so you'll just have to follow along as I do it. However, you could start your own repository and then you'd be able to do similar things.
...
Nothing very formal here, but make sure you're able to install the packages from CRAN (if you're not using the VM) and that you're able to read/write files from an arbitrary directory on your computer's filesystem. Grab one of us if you have any problems.
